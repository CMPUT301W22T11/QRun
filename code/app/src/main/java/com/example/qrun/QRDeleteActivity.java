...
public class QRDeleteActivity extends AppCompatActivity {
    ...
    private ArrayList<QRGame>QRList2=new ArrayList<>();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        QRListView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> adapterView, View view, int i, long l) {
                ...
                //delete same hex QR
                for(int j=0;i<QRList2.size();j++) {
                    if(QRList2.get(j).getHexString().equals(temp.getHexString())){
                    QRGame temp2=QRList2.get(j);
                    QRstorage.delete(temp2,comp);
                }
                }
            }
        });
        ...
        if(queryDocumentSnapshots != null) {
            for (QueryDocumentSnapshot document : queryDocumentSnapshots) {
                ...
                if(document.getId() != QRName) {
                    ...
                    QRList2.add(new QRGame(document));
                }
